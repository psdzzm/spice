...
'''
 ...
 LastEditTime: 10-07-2021 11:24:04
 FilePath: /spice/src/read_copy.py
'''
...
def getfile(root):
    def printdir(currdir):
        print(currdir)
        dir=os.listdir(currdir)
        file=[]
    ...
    currdir=root+'/CirFile'
    printdir(currdir)
    filename = input("Please enter the filename or path (press q to quit): ")
    while True:
        if filename.startswith('.') and not filename.startswith('./') and not filename.startswith('..'):
