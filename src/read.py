...
class circuit:
        ...
        with open(self.name) as file_object:
            for lines in file_object:
                ...
                if row != [] and row[0].lower() not in matches and '.' in row[0].lower():
                    ...
                    path2check = self.workpath+row[1].split('/')[-1].split('.')[0]
                    if (not os.path.isfile(path2check)) and os.path.isfile(row[1]):
                        shutil.copyfile(row[1], path2check)
                        print('Copy '+row[1]+' to '+path2check)
                    self.include.append('.include lib/'+row[1].split('/')[-1].split('.')[0])
                    lines = self.include[-1]+'\n'
    ...
    __count=0
    def fixinclude(self,repl):
        with open('test.cir', 'r+') as file_object, open('run.cir', 'r+') as b:
            test=file_object.read()
            test=test.replace(self.include[self.__count],'.include lib/'+repl)
            file_object.seek(0)
            file_object.write(test)

            run=b.read()
            run=run.replace(self.include[self.__count],'.include lib/'+repl)
            b.seek(0)
            b.write(run)
    ...
    def init(self):
        ...
        if stderr:
            return stderr.decode('ASCII'),False
            ...
            while i < len(fileo):
                if fileo[i].lower().startswith('error'):
                    if fileo[i] == 'Error: no data saved for D.C. Operating point analysis; analysis not run\n':
                        ...
                        self.subckt = fileo[i].split()[-1].upper().split('.')[0]
                        fileo[i] = fileo[i].replace('lib/','') + \
